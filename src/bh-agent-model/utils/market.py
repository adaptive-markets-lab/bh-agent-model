"""
Core insight of the Brock–Hommes model is:
...
class Market:
    """
    Represents the market environment in the Brock–Hommes model.
    ...
    ----------
    traders : list
        List of Trader objects representing available strategies.
    weights : np.ndarray
        Fraction of the population using each strategy.
    beta : float
        Intensity of choice parameter. Controls how strongly agents switch
        toward better-performing strategies.
    R : float
    ...
    noise_std : float
        Standard deviation of random shocks to the market.
    x : float
        Current price deviation from fundamental value.
    """

    def __init__(self, traders, beta, R, sigma2, a, noise_std=0.001):
    ...
    def softmax(self, fitnesses):
        ...
        This implements the evolutionary selection mechanism:
        ...
        Numerical Note
        --------------
        Uses a stabilized version (log-sum-exp trick) to avoid overflow when
        fitness values are large.

        Parameters
        ----------
        fitnesses : np.ndarray
            Array of fitness values for each trader type.
    ...
    def step(self):
        ...
        tuple
            (x_new, weights)
            - x_new : float
                Updated price deviation
            - weights : np.ndarray
                Updated strategy shares
        """
        ...
        # 5. Update weights (strategy switching)
